...
import { setHasMore, setJobLoading, setJobs, setJobsCount } from "@/lib/features/job/jobSlice";
import { Job } from "@/utils/apiHandlers";

const useGetAllJobs = () => {
    const dispatch = useAppDispatch();
    const { API_END_POINT1 } = useAppSelector((state) => state.user);
    const { page, jobs, hasMore } = useAppSelector((state) => state.jobs);
    ...
    useEffect(() => {
        ...
        const fetchJobs = async () => {
            try {
                const res = await axios.get(`${API_END_POINT1}/job/fetch-jobs?page=${page}`, {
