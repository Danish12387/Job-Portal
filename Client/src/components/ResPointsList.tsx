...
const ResPointsList: React.FC<ResPointsListProps> = ({ input, setInput, errorHandler }) => {
    const [textareas, setTextareas] = useState([{ id: 1, value: '' }]);

    useEffect(() => {
        if (input.requirements.length > 0) {
            setTextareas(input.requirements.map((point, index) => ({ id: index + 1, value: point })));
        }
    }, []);
    ...
    const removeTextarea = (id: number) => {
        ...
        setInput({ ...input, responsibilities: newPoints });
    }
    ...
    const handleTextareaChange = (id: number, value: string) => {
        ...
        setInput({ ...input, responsibilities: newPoints2 });
        errorHandler('responsibilities', newPoints2);
    }
